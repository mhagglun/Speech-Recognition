...
def _stft_magnitude_tflite(signals, frame_length, frame_step, fft_length):
    ...
    real_stft, imag_stft = _stft_tflite(
        signals, frame_length, frame_step, fft_length)
    stft_magnitude = tf.sqrt(tf.add(
        real_stft * real_stft, imag_stft * imag_stft), name='magnitude_spectrogram')
...
def waveform_to_log_mel_spectrogram(waveform, params):
    ...
    with tf.name_scope('log_mel_features'):
        ...
        fft_length = 2 ** int(np.ceil(np.log(params.FFT_SIZE) / np.log(2.0)))
        num_spectrogram_bins = fft_length // 2 + 1

        magnitude_spectrogram = _stft_magnitude_tflite(
            signals=waveform,
            frame_length=params.FFT_SIZE,
            frame_step=params.HOP_SIZE,
            fft_length=params.FFT_SIZE)
    ...
    log_mel_spectrogram = tf.math.divide(tf.math.subtract(log_mel_spectrogram, tf.math.reduce_min(
        log_mel_spectrogram)), tf.math.subtract(tf.math.reduce_max(log_mel_spectrogram),
                                                tf.math.reduce_min(log_mel_spectrogram)))
